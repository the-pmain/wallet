...
/**
 * First cabinet screen: role, a name for admin, then the PIN.
 *
 * The server checks the PIN. The form does not know the correct code.
 * The admin name lives in `localStorage` and is filled in automatically.
 */
export function AdminPinForm({
  ...
  const setDigits = (next: string, source: 'keypad' | 'input' = 'keypad') => {
    ...
    /* A password manager dumps the whole PIN in one event after Lock.
       The keypad enters one digit at a time. Ignore the dump so Lock
       cannot sign the operator back in. */
        ...
        <CardContent>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col items-center gap-2.5">
              ...
              {/* PIN is not a browser password. `type="password"` plus a
                  name field makes Chrome save it and fill it after Lock,
                  which signs the operator back in. */}
              <input
                ref={inputRef}
                id={pinId}
                className="sr-only"
